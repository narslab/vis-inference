...
We optimized eight of these parameters using the Hyperband approach \cite{li2018hyperband}.
The others, however, were fixed.
...
The goal of the training procedure is to learn the optimal weights and bias terms for the CNN by minimizing a loss function. In this case, we use the categorical cross-entropy loss function, which is given by:
\begin{equation}
    L_{CE} = - \sum_{c\in C} y \log( f(c)_i)
\end{equation}
The softmax function returns the class prediction probabilities for a given observation $i$. It is defined for each class $c$ as:
\begin{equation}
    f(c)_i = \frac{e^{s^c_i}}{\sum_{c \in C} e^{s^c_i}}
\end{equation}
where $s^c_i$ is class score for the $i$th observation.
Training is iteratively performed, with gradient updates averaged over a batch of input images.
Here, we use a batch size of 32.
The learning rate $\la$ of the optimization algorithm is an important hyperparameter that affects training performance.
...
We performed a guided grid search to find the optimal hyperparameter values across each of the 12 unique classification scenario and image resolution instances.
Each search was conducted using 90 trials of unique hyperparameter combinations. The results of the 8 hyperparameters optimized in each of the 12 training instances is shown in \autoref{tab:opt-hyp}.
