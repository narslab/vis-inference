...
Given that there was no significant difference in performance among the three resolutions tested, we focus our attention on the 64-px case, in the interest of efficiency. We re-trained the model on all the four scenarios and then evaluated performance on the following validation metrics: average precision, average recall and accuracy.
...
In addition to overall accuracy of making correct classifications, we assessed the models trained under these scenarios based on the weighted averages of the precision, recall and $F_1$ score metrics.

\begin{align}
\text{Weighted average precision: }    Pr^w &= \frac{1}{\sum_{c\in C} n_c} \sum_{c\in C} \left(  \frac{n_c TP_{c} }{TP_{c} + FP_c}\right) \\
\text{Weighted average recall: }    Re^w &= \frac{1}{\sum_{c\in C} n_c} \sum_{c\in C} \left(  \frac{n_c TP_{c} }{TP_{c} + FN_c}\right) \\
\text{Weighted average $F_1$ score: }    F_1^w &= \frac{1}{\sum_{c\in C} n_c} \sum_{c\in C} \left(  \frac{2 n_c Pr_{c} Re_{c}}{Pr_c + Re_c}\right)
\end{align}
where the class-based metrics are given by:
\begin{align}
    TP_c &= \sum_{i \in c} I(\hat{y}_i = y_i ) \\
    FP_c &= \sum_{i \in c} I(\hat{y}_i \in c | y_i \notin c) \\
    FN_c &= \sum_{i \in c} I(\hat{y}_i \notin c | y_i \in c)
\end{align}
